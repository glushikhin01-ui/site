...
const PERMISSION_GROUPS = [
  ...
  },
  {
    key: "content",
    title: "Контент и ЧСП",
    perms: ["manage_blacklist", "give_model", "manage_models", "give_weapon", "manage_weapons", "give_job", "manage_jobs", "give_qmenu"]
  },
  {
    key: "comms",
    title: "Коммуникации",
...
function sanitizePerms(j) {
  ...
  for (const role of allowed) {
    ...
    const clean = {};
    for (const k of PERMISSION_KEYS) clean[k] = role === "KP" ? true : Boolean(row[k]);
    out[role] = clean;
  }
  return out;
}
