...
async function ensureIndexes() {
  ...
  const indexes = [
    // ba_bans indexes
    ["ba_bans", "idx_ban_time", "INDEX idx_ban_time (ban_time)"],
    ["ba_bans", "idx_steamid", "INDEX idx_steamid (steamid)"],
    // ba_iplog indexes
    ["ba_iplog", "idx_iplog_steamid", "INDEX idx_iplog_steamid (steamid)"],
    ["ba_iplog", "idx_iplog_lastseen", "INDEX idx_iplog_lastseen (lastseen)"],
    // web_users indexes
    ["web_users", "idx_web_users_role", "INDEX idx_web_users_role (role)"]
  ];

  for (const [table, indexName, ddl] of indexes) {
    try {
      validateId(table, VALID_TABLE_RE);
      const [rows] = await conn.query(
        "SELECT 1 FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND INDEX_NAME = ? LIMIT 1",
        [table, indexName]
      );
      if (!rows.length) {
        await conn.query(`ALTER TABLE ${validateId(table, VALID_TABLE_RE)} ADD ${ddl}`);
        console.log(`[DB] Added index ${indexName} on ${table}`);
      }
    } catch (e) {
      console.error(`[DB] Failed to add index ${indexName}:`, e.message);
    }
  }
}

// FIX: Safe ALTER TABLE with identifier validation
async function safeAddColumn(table, col, def) {
  validateId(table, VALID_TABLE_RE);
  validateId(col, VALID_COL_RE);
  if (!await hasColumn(table, col)) {
    await db().query(`ALTER TABLE ${validateId(table, VALID_TABLE_RE)} ADD COLUMN ${validateId(col, VALID_COL_RE)} ${def}`).catch(() => {});
  }
}
...
async function ensurePanelSchema() {
  ...
  // FIX: Use safeAddColumn instead of raw template literals for ALTER TABLE
  const modelCols = [
  ...
  await conn.query(`CREATE TABLE IF NOT EXISTS tex_info_links (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  await conn.query(`CREATE TABLE IF NOT EXISTS tex_public_links (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    token VARCHAR(64) NOT NULL,
    steamid64 VARCHAR(20) NOT NULL,
    requested_by VARCHAR(128) DEFAULT NULL,
  ...
  const texCols = [
    ["tex_public_links", "discord_guild_id", "VARCHAR(32) DEFAULT NULL"],
    ["tex_public_links", "discord_channel_id", "VARCHAR(32) DEFAULT NULL"],
    ["tex_public_links", "discord_message_id", "VARCHAR(32) DEFAULT NULL"],
    ["tex_public_links", "discord_requester_id", "VARCHAR(32) DEFAULT NULL"],
    ["tex_public_links", "period_days", "INT UNSIGNED DEFAULT NULL"],
    ["tex_public_links", "expires_at", "INT NOT NULL DEFAULT 0"]
  ];
  ...
  await conn.query(`CREATE TABLE IF NOT EXISTS panel_player_access (
    steamid32 VARCHAR(32) NOT NULL,
  ...
  // FIX: Ensure important indexes exist
  await ensureIndexes();
